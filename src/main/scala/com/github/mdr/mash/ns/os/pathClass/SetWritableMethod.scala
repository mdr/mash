...
object SetWritableMethod extends MashMethod("setWritable") {
  ...
  private def calculateNewPermissionSet(value: Boolean,
    ...
    if (value)
      existingPermissionSet ++ parties.writePermissionSet
    else
      existingPermissionSet diff parties.writePermissionSet

  override def typeInferenceStrategy = PermissionsClass

  override def summaryOpt = Some("Set the write bit on the given path")
