...
object LineBufferActionHandler {
  ...
  def unapply(action: InputAction): Option[LineBuffer ⇒ LineBuffer] = condOpt(action) {
    ...
    case BeginningOfLineExtendingSelection ⇒ _.moveCursorToStart(extendSelection = true)
    case EndOfLineExtendingSelection       ⇒ _.moveCursorToEnd(extendSelection = true)
