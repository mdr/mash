...
case class CompletionParser(env: Map[String, MashValue], mish: Boolean) {
  ...
  def parse(s: String): Option[Expr] =
    Compiler.compile(CompilationUnit(s), env, forgiving = true, inferTypes = true, mish = mish)
  ...
  def getBareTokens(s: String): Seq[Token] =
    Compiler.compile(CompilationUnit(s), env, forgiving = true, mish = mish, bareWords = false).map(expr ⇒
