...
object GeneralArgument {
  ...
  case class LongFlag[T](flag: String, valueOpt: Option[T] = None) extends GeneralArgument[T] {
    val isPositionArg = false
  }

  case class ShortFlag(flags: Seq[String]) extends GeneralArgument[Nothing] {
    val isPositionArg = false
    val valueOpt = None
  }
...
case class ArgBindingException[T](message: String, argumentOpt: Option[GeneralArgument[T]] = None) extends RuntimeException(message)

class GeneralArgBinder[T](params: ParameterModel, arguments: Seq[GeneralArgument[T]], ignoreAdditionalParameters: Boolean) {

  private val positionArgs = arguments.collect { case arg: GeneralArgument.PositionArg[T] ⇒ arg }

  private def isProvidedAsNamedArg(name: String): Boolean =
    arguments.exists {
      case GeneralArgument.LongFlag(`name`, _) ⇒ true
      case GeneralArgument.ShortFlag(flags)    ⇒ flags contains name
      case _                                   ⇒ false
    }

  private var boundParams: Set[Parameter] = Set()
  private var parameterToArguments: Map[Parameter, Seq[Option[T]]] = Map()
  ...
  private def addArgToParam(param: Parameter, arg: GeneralArgument[T]) =
    parameterToArguments += param -> (parameterToArguments.getOrElse(param, Seq()) :+ arg.valueOpt)
      ...
      params.variadicParamOpt match {
        ...
        case None                ⇒
          if (!ignoreAdditionalParameters) {
            ...
            throw new ArgBindingException(message, Some(firstExcessArgument))
          }
      }
  ...
  private def handleFlagArgs() {
    for (argument ← arguments)
      argument match {
        case GeneralArgument.ShortFlag(flags)       ⇒
          for (flag ← flags)
    ...
    params.paramByName.get(paramName) match {
      ...
      case None        ⇒
        if (!ignoreAdditionalParameters && !hasNamedArgsParam)
          throw new ArgBindingException(s"Unexpected named argument '$paramName'", Some(arg))
    }
  }
