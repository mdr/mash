...
class ValueTypeDetector {
  ...
  def getType_(x: MashValue): Type = x match {
    ...
    case BoundMethod(target, UserDefinedMethod(_, name, params, _, body, context), _) ⇒ Type.BoundUserDefinedMethod(getType(target), Type.UserDefinedFunction(Some(name), params, body, buildBindings(context.scopeStack.bindings)))
