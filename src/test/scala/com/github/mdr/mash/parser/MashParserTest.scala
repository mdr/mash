...
class MashParserTest extends FlatSpec with Matchers {
  ...
  "A lambda on the left of a pipe" should "bind looser than pipes" in {
    ...
    // x => (x | a)
    parse(s) should equal(
      LambdaExpr(FunctionParamList(Seq(SimpleParam(x))), arrow,
        PipeExpr(
          Identifier(x2),
          pipe,
          Identifier(a))))
  ...
  "A lambda on the left of a pipe" should "bind looser than pipes when nested" in {
    ...
    parse(s) should equal(
      LambdaExpr(
        FunctionParamList(Seq(SimpleParam(x))),
        arr,
        LambdaExpr(
          FunctionParamList(Seq(SimpleParam(y))),
          arr2,
          PipeExpr(
            Identifier(y2),
            pipe,
    ...
    parse(s) should equal(
      PipeExpr(
        PipeExpr(
          Identifier(a),
          pipe,
          LambdaExpr(FunctionParamList(Seq(SimpleParam(x))), arr,
            LambdaExpr(FunctionParamList(Seq(SimpleParam(y))), arr2,
              Identifier(y2)))),
        pipe2,
        Identifier(b)))
  }
