...
class AbstractifierTest extends FlatSpec with Matchers {
  ...
  "Abstract representation of a string literal" should "not include the quotes" in {
    ...
    val abstractExpr = abstractify(concreteExpr, provenance)

    abstractExpr should equal(
      StringLiteral("foo", QuotationType.Double, hasTildePrefix = false, sourceInfoOpt = Some(SourceInfo(provenance, concreteExpr.body))))
  }
  ...
  "A function invocation" should "have its positional arguments collected" in {
    val s = "ls pwd"
    val concreteExpr = parse(s)
    val Concrete.InvocationExpr(function, Seq(arg: Concrete.Expr)) = concreteExpr.body
    val provenance = Provenance(s, source = "test")
    ...
    abstractExpr should equal(
      InvocationExpr(
        ...
        sourceInfoOpt = Some(SourceInfo(provenance, concreteExpr.body))))
  }

  private def abstractify(concreteExpr: Program, provenance: Provenance): Expr =
    new Abstractifier(provenance).abstractify(concreteExpr.body)

  "A function invocation" should "have its flag arguments collected" in {
    val s = "foo --bar=baz"
    val concreteExpr = parse(s)
    val Concrete.InvocationExpr(function, Seq(longArg)) = concreteExpr.body
