...
class DesugarHolesTest extends FlatSpec with Matchers {
  ...
  private def renameHoleVariable(e: Expr): Expr = e.transform {
    case Identifier(DesugarHoles.VariableName, _) ⇒
      Identifier("x", None)
    case LambdaExpr(ParamList(Seq(FunctionParam(attributes, Some(DesugarHoles.VariableName), isVariadic, defaultExprOpt, isLazy, patternOpt, _))), body, _) ⇒
      LambdaExpr(ParamList(Seq(FunctionParam(attributes, Some("x"), isVariadic, defaultExprOpt, isLazy, patternOpt, None))), body, None)
  }
