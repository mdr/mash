...
class CompletionsTest extends FlatSpec with Matchers {
  ...
  "class A { def method = fiel▶ }" shouldNotContainCompletion "fields"
  "class A { def method = this.fiel▶ }" shouldContainCompletion "fields"
  ...
  private implicit class RichString(s: String)(
    ...
    def fullCompletions: Seq[Completion] = {
      val lineBuffer = LineBufferTestHelper.lineBuffer(s)
